...
// Chamando função de salvar IP do usuário nas notas ao realizar o checkout
add_action( 'woocommerce_thankyou', 'add_custom_order_notes_ip_user', 10, 1 );

// Função para salvar nota com IP do user
function add_custom_order_notes_ip_user( $order_id ) {
	
	$order = wc_get_order(  $order_id );
	
	// Conteúdo da nota - ip (obtido com o php)
	$note = __('IP DO COMPRADOR: ' . $_SERVER['REMOTE_ADDR'], 'text-domain');

	// Adicionando e salvando nota
